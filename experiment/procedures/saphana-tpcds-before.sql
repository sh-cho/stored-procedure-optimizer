create or replace procedure "ksc2017_s1" (IN start_year integer, IN end_year integer, OUT first_half_year "TPC".FIRST_HALF , OUT second_half_year "TPC".SECOND_HALF)
AS BEGIN
...
FOR year_cnt in :start_year .. :end_year DO
   FOR month_cnt in 1 .. 12 DO
      _date = select d_date_sk, d_year, d_moy from date_dim  where d_year = :year_cnt and d_moy = :month_cnt;
      IF :month_cnt < 7 THEN -- 상반기
